...
export const updateOrder = asyncHandler(async(req,res,next)=>{
    ...
    const updateOrder = await orderModel.findByIdAndUpdate({_id:orderId},req.body,{new:true})
    return res.status(200).json({message:'Done',updateOrder})
});

export const deleteOrder = asyncHandler(async (req,res,next) => {
    const {orderId} = req.params;
    const deleteOrder = await orderModel.findOneAndDelete({_id:orderId,userId:req.user._id},{new:false})
    return deleteOrder ? res.status(200).json({message:'done', deleteOrder}):next(new Error('In-valid order',{cause:404}))
})
